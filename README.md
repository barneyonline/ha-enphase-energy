...
- Site and battery energy telemetry, including a non-negative Current Power Consumption sensor averaged from Enphase consumption buckets, plus derived grid-import, grid-export, and battery power sensors for Home Assistant Energy Dashboard use. Current Power Consumption retains its last valid reading through invalid or discontinuous samples for at most 15 minutes from the original sample timestamp, then becomes unavailable even if polling stops. Its `sampled_at_utc` attribute preserves that original timestamp. Config-entry diagnostics under `site_energy.consumption_power` identify retained readings and the latest rejected bucket, reason, and baseline; the rejection remains available after recovery until the integration reloads.
