...
- Site and battery energy telemetry, including a non-negative Current Power Consumption sensor averaged from Enphase consumption buckets, plus derived grid-import, grid-export, and battery power sensors for Home Assistant Energy Dashboard use. Current Power Consumption recalculates when valid consumption-energy samples advance and retains its last valid reading through delayed, invalid, or discontinuous samples without an age-based expiry, including when polling stops. Its `sampled_at_utc` attribute preserves that original timestamp. When fresh data returns, consistent bucket changes spanning up to 30 minutes can immediately produce a new average; `last_window_seconds` reports the actual averaging window. Longer gaps still require a new baseline. Config-entry diagnostics under `site_energy.consumption_power` identify retained readings and the latest rejected bucket, reason, and baseline; the rejection remains available after recovery until the integration reloads. `site_energy.fetch` records request attempts, cumulative failures, completion outcomes, and whether source timestamps advance, repeat, regress, are missing, or are too far in the future. Empty or malformed responses with no usable energy flows count as failures and preserve the last successful cache; future timestamps do not advance the diagnostic baseline. These diagnostics distinguish fetch failures from successful responses containing unchanged data without storing raw responses or exception messages.
- IQ Gateway Status attributes include the reported IP address and Connection Method (Ethernet, Wi-Fi, or Cellular). Multiple active methods are listed together; missing connection details leave the method empty. These describe cloud-reported interfaces, not a locally verified default route.
